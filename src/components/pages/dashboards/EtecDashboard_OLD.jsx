...
const EtecDashboard = ({ onLogout }) => {
  ...
  const AddTeacherModal = ({ onBack, onSuccess }) => (
    <CreateTeacher onBack={onBack} onSuccess={onSuccess} />
  )

  const AddStudentModal = ({ onBack, onSuccess }) => (
    <CreateStudent onBack={onBack} onSuccess={onSuccess} />
  )
  ...
  const SettingsModal = () => (
    ...
    >
      <div className="space-y-6">
        ...
        <div className="space-y-3 pt-4 border-t dark:border-[#30363D] border-gray-200">
          {/* Botão de Logout */}
          {onLogout && (
            <button
              onClick={() => {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold dark:text-white text-gray-900">
                {isAluno ? 'Portal do Aluno' : isAdmin ? 'Painel da Secretaria' : etecInfo.name}
              </h1>
        ...
        {showAddTeacherModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl p-6 max-w-lg w-full">
              <CreateTeacher onBack={() => setShowAddTeacherModal(false)} onSuccess={() => setShowAddTeacherModal(false)} />
            </div>
          </div>
        )}
        {showAddStudentModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl p-6 max-w-lg w-full">
              <CreateStudent onBack={() => setShowAddStudentModal(false)} onSuccess={() => setShowAddStudentModal(false)} />
            </div>
          </div>
        )}
        {/* Outros modais mantidos normalmente */}
        <AddClassModal />
        <AddEventModal />
        <AddNewsModal />
        <EditNewsModal />
