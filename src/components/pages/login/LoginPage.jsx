        ...
        <form onSubmit={handleSubmit} className="bg-white/10 backdrop-blur-md p-6 rounded-2xl">
          <div className="space-y-4">
            ...
            <div>
              ...
              <input value={user} onChange={(e) => setUser(e.target.value)} className="w-full p-3 rounded-lg bg-white/10 border border-white/20 text-white" placeholder="RM ou login" required />
            </div>
            <div>
              <label className="text-sm text-white block mb-1">Senha</label>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} value={senha} onChange={(e) => setSenha(e.target.value)} className="w-full p-3 pr-10 rounded-lg bg-white/10 border border-white/20 text-white" placeholder="Senha" required />
            ...
            <div className="flex gap-3">
              <button type="submit" className="flex-1 px-4 py-3 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-lg">Acessar</button>
              <button type="button" onClick={onCancel} className="px-4 py-3 bg-white/10 text-white rounded-lg">Cancelar</button>
            </div>
          </div>
        </form>
      </div>
